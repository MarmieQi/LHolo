...
- 新增“界面设置”页与中英双语界面：原“界面缩放”页更名为“界面设置”，并在其中新增语言选择，默认简体中文。
- 界面文案改为统一的文案键表驱动，中英两张语言表在编译期校验完整性，漏翻或重复会直接编译失败。
- 材料清单中的水与熔岩名称随界面语言切换。
- 投影 HUD、材料 HUD 与结构文件打开/保存对话框的文案一并纳入语言表。
...
- 配置文件版本升级到 11；旧配置缺少新增字段时继续使用默认值。本次新增的 `language` 字段同样属于版本 11，旧配置读取后默认简体中文。
