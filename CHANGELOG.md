...
- 改进与其它覆盖层模组的兼容性：修复同时安装 ChiyanMap 时，用快捷键打开 LHolo 菜单会把鼠标锁死在屏幕中心的问题。根因是菜单使用 ImGui 自绘光标，会把系统光标从屏幕上移除，而其它模组以“系统光标是否显示”判断游戏是否仍抓着鼠标，于是持续把光标夹回客户区中心。现在菜单期间改用系统光标并保持其可见，关闭菜单后恢复由游戏隐藏。
