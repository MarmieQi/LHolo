...
- 默认结构偏移：按住 `Alt` 并滚动滚轮，结构沿视线方向整体移动；按住期间快捷栏锁定，滚轮不会切换手持物品，松开后恢复。触发键固定为 `Alt`，不是按下式热键，不出现在热键页，但可在热键页用“`Alt + 滚轮调整结构位置`”开关整体关闭；关闭后不认领 `Alt`、不接管滚轮、也不锁定快捷栏。
...
“恢复默认快捷键”同时把固定手势开关（`Alt + 滚轮调整结构位置`）恢复为默认开启。该开关是持久化的
